...
zSetDecodedEntriesStreamingDF = zSetEntriesEncodedStreamingDF.withColumn('redisEvent', unbase64(zSetEntriesEncodedStreamingDF.encodedCustomer).cast('string'))
...
emailAndBirthYearStreamingDF = emailAndBirthDayStreamingDF.select('email', split(col('birthDay'), '-').getItem(0).alias('birthYear'))
...
riskScoreByBirthYear = customerRiskStreamingDF.join(emailAndBirthYearStreamingDF, expr('customer = email'))
